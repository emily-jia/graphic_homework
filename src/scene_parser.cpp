...
Light *SceneParser::parsePointLight() {
    ...
    // DONE： 景深相机情况下，读取相机远景模糊参数
    getToken(token);
    double fall_off = 0;
    if(!strcmp(token, "falloff"))
    {
        fall_off = readFloat();
        getToken(token);
    }
    ...
    assert (!strcmp(token, "}"));
    return new PointLight(position, color);  //TODO: 修改为带景深的点光源
}
// ====================================================================
// ====================================================================
...
Object3D *SceneParser::parseObject(char token[MAX_PARSER_TOKEN_LENGTH]) {
    ...
    } else if (!strcmp(token, "BezierCurve")){  // DONE: 读取Beizier曲线，Copy from PA3
        answer = (Object3D*) parseBezierCurve();
    } else if (!strcmp(token, "BsplineCurve")) { // DONE: 读取B样条，Copy from PA3
        answer = (Object3D *) parseBsplineCurve();
    } else if (!strcmp(token, "RevSurface")) {
        answer = (Object3D *) parseRevSurface();
    } else if(!strcmp(token, "SceneGroup")) {  // DONE: 读取带mtl文件的obj文件
        answer = (Object3D *) parseSceneGroup();
    }
    else {
